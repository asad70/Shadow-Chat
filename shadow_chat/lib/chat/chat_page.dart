...
class AppChatPage extends StatefulWidget {
  ...
  @override
  _AppChatPageState createState() => _AppChatPageState();
}
...
class _AppChatPageState extends State<AppChatPage> {
  ...
  StreamSubscription<List<ChatMessagesRecord>> getMessagesStream(
      ...
      AppChatManager.instance.getChatMessages(chatReference).listen((m) {
        if (mounted) {
          updateMessages(m);
          AppChatManager.instance.setLatestMessages(chatReference, messages);
        }
      });
  ...
  void initState() {
    super.initState();
    updateMessages(AppChatManager.instance.getLatestMessages(chatReference));
