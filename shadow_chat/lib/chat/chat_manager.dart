...
class AppChatInfo {
  const AppChatInfo(this.chatRecord, [this.groupMembers]);
...
class AppChatManager {
  AppChatManager._();

  // Cache that will ensure chat queries are kept alive. By default it only keeps
  ...
  static AppChatManager _instance;
  static AppChatManager get instance => _instance ??= AppChatManager._();
  ...
  Stream<AppChatInfo> getChatInfo({
    ...
    return chatStream.asyncMap((chat) async {
      ...
      if (chatRecord != null) {
        ...
        // as this could be computationally expensive. Instead, takes at most
      ...
      return AppChatInfo(chat, groupMembers);
    });
  }
